...
def test_submit_existing_draft():
    with patch("observal_cli.config.resolve_alias") as mock_resolve:
        with patch("observal_cli.client.post") as mock_post:
...
def test_load_from_file(sandbox_payload: dict):
    with patch("builtins.open", mock_open(read_data=_json.dumps(sandbox_payload))):
        with patch("observal_cli.client.post") as mock_post:
            mock_post.return_value = {"id": "sandbox-123"}
            result = runner.invoke(
                app, 
...
def test_draft():
    ...
    with patch("builtins.open", mock_open(read_data=_json.dumps(payload))):
        with patch("observal_cli.client.post") as mock_post:
...
def test_sandbox_install():
    ...
    mock_post.assert_called_once_with("/api/v1/sandboxes/sandbox-123/install",{"ide": "vscode"},)
...
def test_sandbox_edit():
    ...
    mock_post.assert_called_once_with(
        "/api/v1/sandboxes/sandbox-123/start-edit"
    )
